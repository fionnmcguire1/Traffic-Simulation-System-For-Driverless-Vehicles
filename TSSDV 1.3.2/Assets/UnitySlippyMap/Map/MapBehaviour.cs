...
namespace UnitySlippyMap.Map
{
	...
	#region MonoBehaviour implementation
		...
		private void Start ()
		{
			...
			Zoom (0.0f);
			//cameraTransform.position = Vector3(-0.11f, -0.02f, 0.18f);
			//currentCamera.transform.rotation = Quaternion.Euler (-90.0f, 0.0f, 0.0f);
			//currentCamera.transform.rotation = Quaternion.identity;
			//currentCamera.transform.position = new Vector3(-0.11f, -0.02f, 0.18f);
		}
			...
			if (usesOrientation) {
				...
				if (cameraFollowsOrientation) {
					if (lastCameraOrientation == 0.0f) {
						currentCamera.transform.RotateAround (Vector3.zero, Vector3.zero, heading);
						...
						if (relativeAngle > 0.01f) {
							currentCamera.transform.RotateAround (Vector3.zero, Vector3.zero, relativeAngle);
	
							//Debug.Log("DEBUG: cam: " + lastCameraOrientation + ", heading: " + heading +  ", rel angle: " + relativeAngle);
							lastCameraOrientation += relativeAngle;
						} else {
							currentCamera.transform.RotateAround (Vector3.zero, Vector3.zero, heading - lastCameraOrientation);
				...
				if (locationMarker != null
					...
					locationMarker.OrientationMarker.rotation = Quaternion.AngleAxis (heading, Vector3.zero);
				}
			}
